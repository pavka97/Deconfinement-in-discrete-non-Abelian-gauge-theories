...
function main(N::Int64,g::Float64,penalty::Float64,D_max::Int64)
  ...
  if g!=3.
    ...
    f = h5open("states_dir/D3_ground_zero_charge_"*string(N)*"_"*string(index_g-1)*".h5","r")
  ...
  for i in 1:Int(N/2)-2
    push!(states_init, 8) #add the lower state
    push!(states_init, 8) #add the upper state
  end
  ...
  ff = h5open("states_dir/D3_ground_zero_charge_"*string(N)*"_"*string(index_g)*".h5","w")
